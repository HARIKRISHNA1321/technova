...
async def excel_attendance_upload(
    ...
    for username, teacher in state["teachers"].items():
        ...
        # Recompute ALL counters from the complete array using helper
        target_month = date[:7] if date else None
        sync_and_recompute_teacher_stats(teacher, state.get("global_working_days", 22), target_month)
